...
dobInput.setAttribute('min', mnDate());
dobInput.setAttribute('max', mxDate());
...
const dispEntries = () => {
    ...
    const tableEntries = entries.map((entry) => {
        ...
        const a = `<td class="bor">${entry.a}</td>`;

        const row = `<tr>${name} ${email} ${password} ${dateOfBirth} ${a}</tr>`;
        return row;
    }).join("\n");
...
const formSubmit = (event) => {
    ...
    const entry = {
        name, email, password, dob: dateOfBirth, a
    }
